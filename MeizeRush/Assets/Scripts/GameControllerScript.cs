...
public class GameControllerScript : MonoBehaviour
{
    ...
    public float minDistanceToPlayer = 10f; // Distância mínima entre baú e jogador
        ...
        for (int i = 0; i < chestNumber; i++)
        {
            ...
            while (!validPosition)
            {
                ...
                if (Vector3.Distance(playerTransform.position, spawnPos) >= minDistanceToPlayer)
                {
                    ...
                    // Verifica se a nova posição está longe o suficiente de outras posições de baús
